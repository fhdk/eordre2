...
class SettingsDialog(QDialog, Ui_settingsDialog):
    ...
    def __init__(self, settings, employees, parent=None):
        ...
        self.editUserMail.setText(self._settings.order_line["usermail"])
        self.editUserPass.setText(self._settings.order_line["userpass"])
        self.editUserCountry.setText(self._settings.order_line["usercountry"])
        self.editHttp.setText(self._settings.order_line["http"])
        self.editSmtp.setText(self._settings.order_line["smtp"])
        self.editPort.setText(str(self._settings.order_line["port"]))
        self.editMailTo.setText(self._settings.order_line["mailto"])
        self.checkServerData.setChecked(utils.int2bool(self._settings.order_line["sc"]))
        self.editMailServer.setText(self._settings.order_line["mailserver"])
        self.editMailPort.setText(str(self._settings.order_line["mailport"]))
        self.editMailUser.setText(self._settings.order_line["mailuser"])
        self.editMailPass.setText(self._settings.order_line["mailpass"])
        # connect to signals
